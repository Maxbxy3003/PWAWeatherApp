...
function loadSavedActivities() {
    ...
    // loads saved elements, empty array if none
    const savedActivities = JSON.parse(localStorage.getItem('userActivities') || '[]');

    let activitiesHTML = '<h2>Your Activities</h2>';
    
    // If no activities saved, display message to user
    if (savedActivities.length === 0) {
        activitiesHTML = `
        ...
        // Header displayed with count info
        activitiesHTML = `
        ...
        // loops through and displays activities after converting saved activities from array form to string
    ...
    // creates and inserts a nav bar upon first load
