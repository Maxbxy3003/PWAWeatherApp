...
// Main control of the program, starts upon load
document.addEventListener('DOMContentLoaded', mainControl);
...
{
    const savedData = localStorage.getItem('weatherWiseUser');
    // If there is saved data, i.e. pre-registered, skip registration
    if(savedData)
    {
        screenChange('home-screen');
        personalizeWelcome(JSON.parse(savedData));
    }

    else
    {
        screenChange('registration-screen');
        document.getElementById('register-btn').addEventListener('click', userRegistration);
    }
}
...
function personalizeWelcome(user)
{
    ...
    if(welcomeTitle)
    {
        const nickname = user.name.substring(0, 4);
        welcomeTitle.textContent = `Welcome back, ${nickname}, ${user.age}!`;
    }
}
...
function userRegistration()
{
    ...
    // Check for invalid age ranges & invalid syntax
    ...
    // Stores data locally
    localStorage.setItem('weatherWiseUser', JSON.stringify(savedData));

    screenChange('home-screen');
    personalizeWelcome(savedData);

}

// Initializes the function for each of the nav buttons
function setupNavigation()
{
    ...
    document.querySelectorAll('.go-activities').forEach(button =>
    {
        button.addEventListener('click', function()
        {
            screenChange('activities-screen')
            loadSavedActivities();
        });
    });
}
...
function screenChange(screenName)
{
    ...
    // Show the screen requested by the program
...
{
    // Initializes buttons on home screen
    ...
    // Initializes buttons on activity screen
    document.getElementById('save-custom-activity').addEventListener('click', saveCustomActivity);
}

// Easy access to know which screen is currently active
...
async function generateRandomActivity()
{
    ...
    // Ensures its generating from correct weather environment, used to have a bug
    // where it generates based off of previous output
    if(isFreshWeatherData(weatherDisplay))
    {
        originalWeatherCategory = getWeatherCategoryFromText(weatherDisplay);

    //Needs to base it off a location to run
    }
    ...
    try
    {
        if(currentActivityType === 'favourite')
            ...
            // Filters to specific current weather & any weather
            ...
            // If no appropriate weather/any weather saved, return
            ...
            // Randomly generates and outputs
        ...
        {
            // Random activity generator based on database.
...
// Helper function to ensure its generating based off of correct data
...
// Helper function v2 that simplifies possible weather conditions into 6.
function getWeatherCategoryFromText(weatherText)
{
    ...
    switch(true)
    {
        case weatherLower.includes('rain')||weatherLower.includes('drizzle'):
        {
            return 'rainy';
        }

        case weatherLower.includes('snow')||weatherLower.includes('freezing'):
        {
            return 'snowy';
        }

        case weatherLower.includes('cold'):
        {
            return 'cold';
        }
        
        case weatherLower.includes('wind'):
        {
            return 'windy';
        }
        
        case weatherLower.includes('cloud') || weatherLower.includes('overcast') || weatherLower.includes('fog'):
        {
            return 'cloudy';
        }
        
        case weatherLower.includes('sun') || weatherLower.includes('clear') || weatherLower.includes('warm'):
        {
            return 'sunny';
        }
...
// Generated list of activities database for random activity generation
function getLocalActivityByWeather(weatherCategory)
{
    ...
    // Chooses a random activity based on the current weather patterns
    const activities = activityDatabase[weatherCategory] || activityDatabase.general; //general as a fallback
    const randomIndex = Math.floor(Math.random() * activities.length);
    return activities[randomIndex];
}

// Saves user added activities to localstorage
function saveCustomActivity()
{
    //Finds the text input and removes extra spaces at the back
    const activityInput = document.getElementById('custom-activity-input');
    const activity = activityInput.value.trim();
    
    //Error prevention if activity is empty
    if(!activity)
    {
    ...
    //Loops through each selected weather type and adds it to the storage
    ...
    //Error prevention 2, to ensure user selects a weather type
    ...
    // Load existing entries into an array, empty array if theres nothing
    const savedActivities = JSON.parse(localStorage.getItem('userActivities') || '[]');
    
    // Add new activity into storage
    ...
    // s=Save new entry as a string into storage
    localStorage.setItem('userActivities', JSON.stringify(savedActivities));
    
    // Clear input and show success alert
    activityInput.value = '';
    alert('Activity saved successfully!');

}

// displays saved activities on activities screen
